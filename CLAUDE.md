...
- **Routes:** `/` (landing) · `/passport` (compliance flow) · `/deal-room` (gated app) · `/markets` (the ComplianceHook demo — both v4 pools, per-actor claims and access, issuer verify/revoke, the on-chain log and the tx inspector; replaces the retired `apps/hook-demo`) · `/concierge` (the agent demo, a placeholder until plan task 7 lands).
- **Shared demo UI (`src/components/demo/`):** `useDemoWorld` (the `/api/demo/world` poll + the world types), `ActorCard` (label/value rows + action slot — it knows no vocabulary of its own), `StatusPill`, `ReasonBadge` (the refusal code, verbatim), `ActionButton`, `TxLog` and `TxInspector`. `/markets` and `/concierge` both build on these. Log lines are **structured, never HTML** — chain data (reason codes, event args) is rendered as text, never through `dangerouslySetInnerHTML`.
...
- **One palette, no exceptions:** navy `#0D1428` on `#F0F2F6`, gradient `#4A9EFF → #3DDBD9`, Inter + JetBrains Mono, white cards with `#DDE1EA` borders, uppercase tracking-widest cyan eyebrows, body text `#4B5568` / muted `#9CA3AF`. It is declared in `tailwind.config.ts` (`brand.*`, `surface.*`, `content.*`) but every page and the shell actually write it as inline arbitrary hex (`bg-[#F0F2F6]`) — so match the hex you see in a neighbouring component, and do not introduce a colour that is not in that list. Semantic states reuse Tailwind defaults (`red-*` errors, `amber-*` warnings). No page ships its own header or footer — the shell owns both.
