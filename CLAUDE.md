...
- **Routes:** `/` (landing) · `/passport` (compliance flow) · `/deal-room` (gated app) · `/markets` (the ComplianceHook demo — both v4 pools, per-actor claims and access, issuer verify/revoke, the on-chain log and the tx inspector; replaces the retired `apps/hook-demo`) · `/concierge` (the agent demo — house header, agent standing, owner KYC controls, ticket composer + feed, approval queue, mandate/fund controls; replaces the retired `apps/concierge`). Both demo routes are hidden and 403 unless `DEMO_MODE=true`.
