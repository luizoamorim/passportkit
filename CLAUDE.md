...
- Design tokens live in `tailwind.config.ts` and nowhere else: navy `#0D1428` on `#F0F2F6`, gradient `#4A9EFF → #3DDBD9`, Inter + JetBrains Mono, white cards with `#DDE1EA` borders, uppercase tracking-widest cyan eyebrows. No page ships its own palette or its own header/footer.
